...
Imports Nancy.Json
Imports JavaScriptSerializer = System.Web.Script.Serialization.JavaScriptSerializer
...
Public Class AfricasTalkingGateway
    ...
    Private Shared PREMIUM_SMS_URLString As String = "https://content.africastalking.com/version1/messaging"
    ...
    Public Function sendMessage(to_ As String, message_ As String, Optional from_ As String = Nothing, Optional bulkSMSMode_ As Integer = 1, Optional options_ As Hashtable = Nothing) As Object
        ...
        If options_ IsNot Nothing Then
            response = sendPostRequest(data, PREMIUM_SMS_URLString)
        Else
            response = sendPostRequest(data, SMS_URLString)
        End If

        UtilityFunctions.LogText(response)
        ...
        If responseCode = HttpStatusCode.Created Then
            ...
            Dim Status As Object = TryCast(recipients(0), Object)
            Dim responseMessage As String = Status("status") 'recipients(0) 'smsMessageData("Message")
            'Return recipients 'Also change this function return type to Object() when uncomment this...
    ...
    'Messaging functions
    Public Function sendMultipleMessages(to_ As String, message_ As String, Optional from_ As String = Nothing, Optional bulkSMSMode_ As Integer = 1, Optional options_ As Hashtable = Nothing) As Object()
        Dim data As New Hashtable()
        data("username") = _username
        data("to") = to_
        data("message") = message_
    ...
    Public Function fetchMessages(lastReceivedId_ As Integer) As Object()
        ...
        If responseCode = HttpStatusCode.OK Then
            Dim json As Object = serializer.DeserializeObject(response)
            Dim smsMessageData As Object = json("SMSMessageData")
            Dim data As Object() = TryCast(smsMessageData("Messages"), Object())
            Return data
        End If
        Throw New AfricasTalkingGatewayException(response)
    End Function
    ...
    'Voice functions
    Public Sub [call](from_ As String, to_ As String)
        Dim data As New Hashtable()
        data("username") = _username
        data("from") = from_
        data("to") = to_
        ...
        If jsObject("Status") <> "Success" Then
            Throw New AfricasTalkingGatewayException(jsObject("ErrorMessage"))
        End If
    End Sub

    Public Function getNumQueuedCalls(phoneNumber_ As String, Optional queueName_ As String = Nothing) As Integer
        Dim data As New Hashtable()
        data("username") = _username
        ...
        Dim QueuedCalls As Integer = jsObject("NumQueued")
        Return QueuedCalls
    End Function
    ...
    Private Function sendPostRequest(dataMap_ As Hashtable, urlString_ As String) As String
        Try
            ...
            webRequest__1.Timeout = 3600000 '1 hr
